...
const ConnectionRequestsTab = () => {
  ...
  const fetchEquipmentOptions = async () => {
    try {
      ...
      const response = await api.get("/equipment/", {
        ...
        params: {
          page_size: 100, // Fetch more for the dropdown
        },
      });

      const equipmentArray = response.data.results || response.data || [];
  ...
  const handleCreateContract = async () => {
    try {
      ...
      await api.post(
        `/connection-requests/${contractData.connection_request}/approve/`,
        contractData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      // Refresh the request data
      const response = await api.get(
        `/connection-requests/${contractData.connection_request}/`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
