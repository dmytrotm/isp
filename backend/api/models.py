...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def get_full_name(self):
        """
        Return the last_name plus the first_name, with a comma and space in between.
        """
...
class Customer(models.Model, ContextAwareModelMixin):
    ...
    balance_negative_since = models.DateTimeField(null=True, blank=True)
...
class Tariff(models.Model):
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2)
    description = models.TextField()
    speed_mbps = models.IntegerField(default=100)
    traffic_limit_gb = models.IntegerField(default=1000)
    is_active = models.BooleanField(default=True)
...
def get_default_connection_request_status():
    try:
        # Get or create the context first to avoid complex lookups during migration
        context, _ = StatusContext.objects.get_or_create(context='ConnectionRequest')
        status, _ = Status.objects.get_or_create(status='New', context=context)
        return status.id
    except Exception:
        return 1
...
class ContractEquipment(models.Model):
    ...
    def return_equipment(self):
        """Marks equipment as returned and updates stock quantity"""
        if self.is_active:
...
class Payment(models.Model):
    ...
    def apply_to_balance(self):
        ...
        record_transaction(
            ...
            description=f'Payment #{self.id} applied'
        )
...
class SupportTicket(models.Model):
    ...
    def assign_employee(self, employee):
        ...
        try:
            open_status = Status.objects.get(status='Open', context__context='SupportTicket')
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            ...
            hours = SLA_HOURS.get(self.ticket_type, 24)
            # Use current time as default start for SLA if not already set
            start_time = now()
            self.sla_deadline = start_time + timedelta(hours=hours)
...
class ClientScore(models.Model):
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='scores')
    score = models.IntegerField()
    contract_points = models.IntegerField()
    overdue_points = models.IntegerField()
    ticket_points = models.IntegerField()
    ...
    def __str__(self):
        return f"Score {self.score} for {self.customer.user.email} at {self.calculated_at}"

class TariffRecommendation(models.Model):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='tariff_recommendations')
    current_tariff = models.ForeignKey(Tariff, on_delete=models.SET_NULL, null=True, related_name='recommendations_as_current')
    recommended_tariff = models.ForeignKey(Tariff, on_delete=models.SET_NULL, null=True, related_name='recommendations_as_suggested')
    reason = models.CharField(max_length=20, choices=REASON_CHOICES)
    avg_usage_percent = models.DecimalField(max_digits=5, decimal_places=2)
    ...
    def __str__(self):
        return f"Recommendation ({self.reason}) for {self.customer.user.email}"
...
class Notification(models.Model):
    ...
    TYPE_CHOICES = [
        ...
        (TARIFF_RECOMMENDATION, 'Tariff Recommendation'),
    ]
    ...
    def __str__(self):
        return f"Notification ({self.notification_type}) for {self.customer.user.email}"
